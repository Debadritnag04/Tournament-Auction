...
export default function LandingPage() {
  const { setStep, players, resetForNewAuction } = useStore();

  // Check if there's an ongoing auction (players loaded and some are sold/available)
  const hasOngoingAuction = players.length > 0;
  ...
  return (
    <div className="min-h-screen bg-[#030305] text-white font-sans flex flex-col relative overflow-hidden">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 pt-32 pb-20 flex-1 flex flex-col items-center justify-center text-center">
        <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} aria-label="Hero">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            {hasOngoingAuction && (
              <button
                ...
                aria-label="Continue the ongoing auction"
                className="px-10 py-5 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-lg uppercase tracking-[0.2em] hover:scale-105 transition-all shadow-[0_0_40px_rgba(16,185,129,0.3)] hover:shadow-[0_0_60px_rgba(16,185,129,0.5)] flex items-center gap-3 cursor-pointer"
              >
                <RotateCcw className="w-6 h-6" /> Continue Auction
              </button>
            )}
            <button
              onClick={() => resetForNewAuction()}
              aria-label="Launch a new auction setup wizard"
              className="px-10 py-5 bg-[#f27d26] hover:bg-[#d96a1a] text-black font-black text-lg uppercase tracking-[0.2em] hover:scale-105 transition-all shadow-[0_0_40px_rgba(242,125,38,0.3)] hover:shadow-[0_0_60px_rgba(242,125,38,0.5)] flex items-center gap-3 cursor-pointer"
            >
              <Play className="w-6 h-6 fill-current" /> New Auction
            </button>
          </div>
        </motion.section>
        ...
        >
          {[
            ...
            <article key={title} className="bg-white/[0.03] border border-white/5 p-6 hover:border-[#f27d26]/30 transition-colors">
