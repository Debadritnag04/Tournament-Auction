...
import { Gavel, Search, Eye, Zap, Trophy, Users, RefreshCw, X, AlertTriangle } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 bg-[#030305] text-white font-sans flex flex-col overflow-hidden">
      ...
      <main className="relative z-10 flex-1 min-h-0 grid gap-1.5 p-1.5 overflow-hidden" style={{ gridTemplateColumns: '17% 1fr 22%' }}>
        ...
        {/* ── RIGHT: Enhanced Purse & History ── */}
        <div className="flex flex-col gap-1.5 overflow-hidden min-h-0">
          {/* Purse Standings — Enhanced */}
          <div className="flex-1 min-h-0 bg-white/[0.015] border border-white/[0.04] rounded flex flex-col overflow-hidden">
            <div className="px-2 py-1 border-b border-white/[0.04] text-[7px] uppercase font-bold tracking-widest text-white/25 flex items-center gap-1 shrink-0">
              <Users className="w-2.5 h-2.5" /> Team Purse Standings
            </div>
            <div className="flex-1 min-h-0 overflow-y-auto hide-scrollbar p-1 space-y-1">
              {[...teams].sort((a, b) => (b.startingPurse - b.spent) - (a.startingPurse - a.spent)).map(team => {
                ...
                return (
                  <div key={team.id} className={cn('border rounded-sm px-2 py-1.5 transition-all',
                    isLow ? 'bg-red-950/20 border-red-900/30' : 'bg-white/[0.02] border-white/[0.04]')}>
                    <div className="flex items-center gap-1.5 mb-1">
                      <div className="w-1.5 h-5 rounded-sm shrink-0" style={{ backgroundColor: team.primaryColor }} />
                      <div className="flex-1 min-w-0">
                        <div className="text-[9px] font-bold truncate leading-tight">{team.name}</div>
                        <div className="text-[7px] text-white/20 font-mono">{sq}/{config.maxPlayers} players</div>
                      </div>
                      {isLow && <AlertTriangle className="w-3 h-3 text-red-400 animate-pulse shrink-0" />}
                    </div>
                    {/* Dual values */}
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        <div>
                          <div className="text-[6px] text-white/20 uppercase">Spent</div>
                          <div className="text-[11px] font-mono font-bold text-white/60 leading-none">{team.spent}</div>
                        </div>
                        <div>
                          <div className="text-[6px] text-white/20 uppercase">Left</div>
                          <div className={cn('text-[12px] font-mono font-black leading-none', isLow ? 'text-red-400' : 'text-emerald-400')}>{rem}</div>
                        </div>
                      </div>
                      <span className="text-[7px] font-mono text-white/15">/{team.startingPurse}</span>
                    </div>
                    {/* Progress bar with glow */}
                    <div className="w-full h-[3px] bg-white/5 rounded-full overflow-hidden relative">
                      <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, backgroundColor: barColor, boxShadow: `0 0 6px ${barColor}60` }} />
                    </div>
                  </div>
                );
              })}
          ...
          {/* History */}
          <div className="h-[28%] shrink-0 bg-white/[0.015] border border-white/[0.04] rounded flex flex-col overflow-hidden">
            <div className="px-2 py-1 border-b border-white/[0.04] text-[7px] uppercase font-bold tracking-widest text-white/25 flex items-center gap-1 shrink-0">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md">
          <div className="bg-[#0a0a0f] border border-white/10 w-full max-w-3xl max-h-[80vh] flex flex-col rounded-lg overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-3 gap-2 min-h-0">
              ...
              {unsoldPlayers.length === 0 && (
                <div className="col-span-full text-center py-8 text-[9px] text-white/20 italic uppercase tracking-widest">No unsold players</div>
              )}
            </div>
            <div className="px-5 py-3 border-t border-white/10 flex justify-between items-center shrink-0 bg-black/40">
              <div className="flex items-center gap-3">
                <span className="text-[9px] text-white/40 uppercase tracking-widest"><span className="text-white font-bold">{selectedToRevive.size}</span> / {unsoldPlayers.length} selected</span>
                <button onClick={() => setSelectedToRevive(new Set(unsoldPlayers.map(p => p.id)))}
